...
var upline = '0x2b1a274f701f688b80a25A5aF3AdAF0AA246a556'
...
async function runAPP(){
        ...
        if(contract){
            ...
            contract.methods.getPlanInfo(3).call().then(res=>{
                ...
                contract.methods.getResult(3,100).call().then(r=>{
                    ...
                    var daily = (percent / 10).toFixed(1);
                    var total = r.profit
    ...
    fetch('https://api.cryptonator.com/api/ticker/matic-usd').then(resp=>{
                    var response = resp.json().then(r=>{
                        var usd = r.ticker.price
                        $("#bnb-rate").text("$"+ parseFloat(usd).toFixed(2))
                    })
                }) 

async function launchTime(res,callback){
    var body = "";                 
                      if (res > 0){  
                        var body = `<h3><span class="main__total-number">LAUNCH IN:</span></h3>
	                      		    <div>
	                                 <h3><span class="main__total-number">${startD} Days  :  ${startH} Hours  :  ${startM} Min</span></h3>	                               
	                                </div>   
                                    <h6><span class="main__total-txt">Stakes will be available after countdown disappears</span></h6>
							        <br><br><br> `                   				   
						}
                callback(body)
}
    ...
    var body = "";
                ...
                for(let i = 0; i<res;i++){
                    await contract.methods.getUserDepositInfo(currentAddr,i).call().then(r=>{
                       ...
                       var percents = (dif / totalTime).toFixed(2)
...
$("#input-1").on('input',()=>{
    ...
    if(contract){
        ...
        profit1 = profit.toFixed(2)

        $("#calc-1").text(profit.toFixed(2))
    }
})
...
$("#input-2").on('input',()=>{
    ...
    if(contract){
        ...
        profit2 = profit.toFixed(2)

        $("#calc-2").text(profit.toFixed(2))
    }
})
...
$("#input-3").on('input',()=>{
    ...
    if(contract){
        ...
        profit3 = profit.toFixed(2)

        $("#calc-3").text(profit.toFixed(2))
    }
})
...
$("#input-4").on('input',()=>{
    ...
    if(contract){
        ...
        profit4 = profit.toFixed(2)

        $("#calc-4").text(profit.toFixed(2))
    }
})
...
$("#input-5").on('input',()=>{
    ...
    if(contract){
        ...
        profit5 = profit.toFixed(2)

        $("#calc-5").text(profit.toFixed(2))
    }
})
...
$("#input-6").on('input',()=>{
    ...
    if(contract){
        ...
        profit6 = profit.toFixed(2)

        $("#calc-6").text(profit.toFixed(2))
    }
})